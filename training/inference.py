...
def count_relate(img, model, processor):
    ...
    data_pro = processor(
        images=img, text=attribute_total, return_tensors="pt", padding=True
    ).to(model.device)
...
def inference(arg, emotion):
    ...
    def save_pic(emotion, img, path, semantic, score):
        ...
        try:
            files = sorted(
                [x for x in os.listdir(path) if x.endswith("_v2.jpg")],
    ...
    text_encoder = CLIPTextModel.from_pretrained(
        "/mnt/d/EmoGen/runs/emoclip_caption/checkpoint-2500"
    )
...
def generate(
    ...
    parser.add_argument("--num_picture", type=int, default=100)
    parser.add_argument(
        "--repo_id", type=str, default="/mnt/d/models/stable-diffusion-v1-5/"
    )
...
if __name__ == "__main__":
    ...
    weight = (
        "/mnt/d/EmoGen/weights/Clip_emotion_classifier/time_2023-11-12_03-29-best.pth"
    )
