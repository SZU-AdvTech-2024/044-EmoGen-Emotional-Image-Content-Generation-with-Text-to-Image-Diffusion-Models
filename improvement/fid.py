...
# # 加载图像函数，支持随机采样
# def load_images_from_folder(folder, image_size=299, max_images=None):
...
#         images = random.sample(images, max_images)  # 随机采样
#     return torch.stack(images) if images else None


# # 提取特征
# def extract_features(images, model, batch_size=32, device="cuda"):
#     model.eval()
#     features = []
#     with torch.no_grad():
...
# # 多类别 FID 计算
# def compute_fid_for_multiclass(folder1, folder2, batch_size=32, device="cuda"):
#     # 加载 InceptionV3 模型
#     model = inception_v3(pretrained=True).to(device)
#     model.fc = torch.nn.Identity()  # 移除全连接层

#     # 遍历类别子文件夹
#     categories = os.listdir(folder1)
#     fid_scores = []

#     for category in categories:
#         folder1_path = os.path.join(folder1, category)
#         folder2_path = os.path.join(folder2, category)

#         if not os.path.exists(folder1_path) or not os.path.exists(folder2_path):
#             print(
#                 f"Skipping category {category} as it is missing in one of the folders."
#             )
#             continue
...
#         # 确定采样数量（取较少文件夹的图片数量）
#         min_images = min(len(os.listdir(folder1_path)), len(os.listdir(folder2_path)))

#         # 重新加载图像，随机采样
#         images1 = load_images_from_folder(folder1_path, max_images=min_images)
#         images2 = load_images_from_folder(folder2_path, max_images=min_images)

#         if images1 is None or images2 is None:
#             print(f"Skipping category {category} due to empty folder.")
#             continue

#         # 提取特征
#         features1 = extract_features(images1, model, batch_size, device)
#         features2 = extract_features(images2, model, batch_size, device)
...
#         print(f"FID for category {category}: {fid}")
#         fid_scores.append(fid)

#     # 输出整体 FID
#     avg_fid = np.mean(fid_scores)
#     print(f"Average FID across all categories: {avg_fid}")
#     return avg_fid
...
#     folder1 = "/mnt/d/data/EmoSet_v5_train-test-val/image"
#     folder2 = "/mnt/d/EmoGen/runs/test1_emoclip2500_5/5/img"
#     fid_score = compute_fid_for_multiclass(folder1, folder2, device=device)
#     print(f"Overall FID score: {fid_score}")
...
def compute_activation_statistics(
    files, model, batch_size=50, device="cpu", num_workers=1
):
...
def compute_fid(path_to_stylized, path_to_style, batch_size, device, num_workers=1):
    ...
    mu1, sigma1 = compute_activation_statistics(
        stylized_image_paths, model, batch_size, device, num_workers
    )
    mu2, sigma2 = compute_activation_statistics(
        style_image_paths, model, batch_size, device, num_workers
    )
...
#     path_to_stylized, path_to_style, batch_size, device, num_points=15, num_workers=1
# ):
...
#     assert len(stylized_image_paths) == len(
#         style_image_paths
...
#         start=5000, stop=len(stylized_image_paths), num=num_points
#     ).astype("int32")
...
#         mu_style, sigma_style = np.mean(act_style_batch, axis=0), np.cov(
#             act_style_batch, rowvar=False
#         )
#         mu_stylized, sigma_stylized = np.mean(act_stylized_batch, axis=0), np.cov(
#             act_stylized_batch, rowvar=False
#         )

#         fid_value = compute_frechet_distance(
#             mu_style, sigma_style, mu_stylized, sigma_stylized
#         )
...
def compute_content_distance(
    ...
    dataset_stylized = ImagePathDataset(
        stylized_image_paths, transforms=content_transforms
    )
    ...
    dataset_content = ImagePathDataset(
        content_image_paths, transforms=content_transforms
    )
...
def compute_art_fid(
    ...
    if mode == "art_fid_inf":
        fid_value = compute_fid_infinity(
            path_to_stylized, path_to_style, batch_size, device, num_workers
        )
    else:
    ...
    content_dist = compute_content_distance(
        path_to_stylized,
        path_to_content,
        batch_size,
        content_metric,
...
if __name__ == "__main__":
    ...
    batch_size = 256
    num_workers = 4
    fid_value = compute_fid(folder1, folder2, batch_size, device, num_workers)
